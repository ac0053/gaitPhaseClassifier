...
class InverseKinematics:
    def __init__(self,model,data,num_rot,num_trans,leg_names,effectors,joint_names,p_base,stance_duty,max_body_trans,max_body_rot,swing_duration,step_height,samps_per_step):
        self.model=model
        self.data=data
    ...
    def stance_vec_calc(self,r_foot_wrt_base,rot_mat,trans_mat,leg,tran,rot):
        ...
        r_one_frame = np.array(
            ...
            [math.sin(psi / (self.num_samps_per_step - 1)),math.cos(psi / (self.num_samps_per_step - 1)),0],[0,0,1],])
    ...
    def foot_trajectory(self,leg,stance_vec):
        ...
        a5_13 = np.array(
                 [[1, t0, t0 ** 2, t0 ** 3, t0 ** 4, t0 ** 5], [0, 1, 2 * t0, 3 * t0 ** 2, 4 * t0 ** 3, 5 * t0 ** 4],
                  ...
                  [1, tf, tf ** 2, tf ** 3, tf ** 4, tf ** 5], [0, 1, 2 * tf, 3 * tf ** 2, 4 * tf ** 3, 5 * tf ** 4],
        ...
        a5_12 = np.array(
                 [[1, t0, t0 ** 2, t0 ** 3, t0 ** 4, t0 ** 5], [0, 1, 2 * t0, 3 * t0 ** 2, 4 * t0 ** 3, 5 * t0 ** 4],
                  ...
                  [0, 0, 0, 6 * tf, 24 * tf ** 2, 60 * tf ** 3]])
        a5_23 = np.array([[1, t_mid, t_mid ** 2, t_mid ** 3, t_mid ** 4, t_mid ** 5],
                               [0, 1, 2 * t_mid, 3 * t_mid ** 2, 4 * t_mid ** 3, 5 * t_mid ** 4],
        ...
        swing_x_vel = (x_min - x_max) / self.swing_duration
        swing_x_acc = 0
    ...
    def moore_penrose(self,leg,r_foot_wrt_base,foot_path):
        ...
        step_frequency = 1 / (2 * self.swing_duration)
        ...
        while ind < num_steps and to_continue:
            ...
            r_foot[:, ind] = effector_position - self.p_base

            mujoco.mj_jacBody(self.model, self.data, jacp, jacr, body_id)
            jac_active = jacp[:, joint_id]

            a = jac_active @ jac_active.T
            b = foot_path[:, ind] - foot_path[:, ind - 1]

            # singularity Check
            if np.linalg.matrix_rank(a) < 3 or np.linalg.cond(a) > 250:
                ik_config_change = jac_active.T @ np.linalg.solve(a + 0.01*np.eye(3), b)
            else:
                # solve for the IK movement before applying penalty
                ik_config_change = jac_active.T @ np.linalg.solve(a, b)
            ...
            while gamma >= tol:
                ...
                penalty = -1/(np.maximum(current_q - lower_limit, tol)) + 1/(np.maximum(upper_limit - current_q, tol))
    ...
    def kinematics_scale(self,kinematics,foot_path,foot_path_st_f_new,foot_path_sw_new_5th,step_frequency):
        ...
        sw_kinematics = kinematics[:,range(foot_path_st_f_new.shape[1], foot_path_st_f_new.shape[1] + foot_path_sw_new_5th.shape[1])]
        ...
        foot_path_resample = np.concatenate((foot_path_new_5th_st1, foot_path_new_5th_sw, foot_path_new_5th_st2),axis=1)
